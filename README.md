...
This project provides an interactive web dashboard for visual perception using a Raspberry Pi 4 camera system connected to a GPU-enabled PC. The Raspberry Pi handles camera capture, image and video upload, media playback, and the web interface, while the main PC performs AI inference using multiple vision models. The dashboard supports live camera input, uploaded images, and paused frames from uploaded videos. Video frames can be selected interactively and analyzed using the same CLIP, YOLO, SAM 2, and Gemma 3 pipeline used for still images.
